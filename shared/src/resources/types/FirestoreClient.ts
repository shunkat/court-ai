import { where, Unsubscribe, DocumentReference } from "@firebase/firestore";
import {
  ListenListCallback,
  _create,
  _get,
  _listenList,
} from "../../utils/Firestore/ClientActions";
...
export namespace Client {
  ...
  export async function createUser(data: User) {
    const { name } = data;
    const test = await _create({ name }, COLLECTIONS.User);
  }
  ...
  export function listenMessages(
    ...
    return _listenList<Message>(
      ...
      [
        ...
        where("updatedAt", ">", Timestamp.fromDate(startUpdatedAt)),
      ]
    );
  }
}
